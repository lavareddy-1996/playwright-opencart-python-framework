...
class CheckoutPage:
    ...
    def get_order_confirmation_message(self):
        """
        Return the order confirmation heading locator, so tests can assert
        on it (e.g. expect(...).to_contain_text("Your order has been placed")).

        Note: unexpected dialog/alert auto-accept is handled once, globally,
        by the `page` fixture in conftest.py rather than here — registering
        a page.on("dialog", ...) listener inside a method that may be called
        more than once (e.g. under pytest-rerun failures) would stack up
        duplicate listeners.
        """
        try:
